...
export class DoctorController {
  static async getAllDoctors(req: Request, res: Response) {
    try {
      ...
      const doctors = db.prepare(`
        SELECT 
          u.id as userId, u.fullName, u.nationalCode, u.mobile, u.email,
  ...
  static async getDoctorById(req: Request, res: Response) {
    try {
      ...
      const doctor = db.prepare(`
        SELECT u.id as userId, u.fullName, u.nationalCode, u.mobile, u.email,
               u.isActive, u.createdAt,
               d.id as doctorId, d.specialty, d.biography,
               d.experienceYears, d.consultationFee, d.rating
        FROM tbl_users u
  ...
  static async createDoctor(req: Request, res: Response) {
    try {
      ...
      const existing = db.prepare('SELECT * FROM tbl_doctors WHERE userId = ?').get(userId) as any;
      if (existing) {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `).run(userId, specialty || 'تعیین نشده', biography || null, experienceYears || 0, consultationFee || 0);

      const doctor = db.prepare(`
        SELECT u.id as userId, u.fullName, u.nationalCode, u.mobile, u.email, u.isActive,
               d.id as doctorId, d.specialty, d.biography, d.experienceYears, d.consultationFee, d.rating
        FROM tbl_users u LEFT JOIN tbl_doctors d ON d.userId = u.id WHERE u.id = ?
      `).get(userId) as any;

      res.status(201).json({
        message: 'اطلاعات پزشک با موفقیت ثبت شد',
        doctor: {
          id: doctor.userId,
          userId: doctor.userId,
          fullName: doctor.fullName || 'نامشخص',
    ...
    try {
      ...
      const patients = db.prepare(`
        SELECT DISTINCT 
          ...
          (SELECT COUNT(*) FROM tbl_appointments a2 WHERE a2.patientId = u.id AND a2.doctorId = ?) as appointmentCount,
          (SELECT MAX(a3.fdate) FROM tbl_appointments a3 WHERE a3.patientId = u.id AND a3.doctorId = ?) as lastVisit
        FROM tbl_users u
        INNER JOIN tbl_appointments a ON a.patientId = u.id
        WHERE u.role = 'patient' AND u.isActive = 1 AND a.doctorId = ? AND u.clinicId = ?
        ORDER BY lastVisit DESC
      `).all(doctorId, doctorId, doctorId, clinicId) as any[];
