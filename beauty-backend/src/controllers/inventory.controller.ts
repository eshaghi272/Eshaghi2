...
export class InventoryController {
  ...
  static async getAll(req: Request, res: Response) {
    try {
      ...
      let query = 'SELECT * FROM tbl_inventory WHERE clinicId = ?';
  ...
  static async getById(req: Request, res: Response) {
    try {
      ...
      const item = db.prepare('SELECT * FROM tbl_inventory WHERE id = ? AND clinicId = ?').get(id, clinicId) as any;
    ...
    try {
      ...
      const stmt = db.prepare(`
        INSERT INTO tbl_inventory (
          clinicId, productName, category, quantity, minThreshold, unitPrice, supplier, fdate
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      const item = db.prepare('SELECT * FROM tbl_inventory WHERE id = ?').get(result.lastInsertRowid) as any;
  ...
  static async update(req: Request, res: Response) {
    try {
      ...
      const existing = db.prepare('SELECT * FROM tbl_inventory WHERE id = ? AND clinicId = ?').get(id, clinicId) as any;
      if (!existing) {
        return res.status(404).json({ message: 'قلم موجودی یافت نشد' });
      }
      ...
      const item = db.prepare('SELECT * FROM tbl_inventory WHERE id = ?').get(id) as any;
  ...
  static async delete(req: Request, res: Response) {
    try {
      ...
      const result = db.prepare('DELETE FROM tbl_inventory WHERE id = ? AND clinicId = ?').run(id, clinicId);
  ...
  static async getCategories(req: Request, res: Response) {
    try {
      ...
      const categories = db.prepare(`
        ...
        FROM tbl_inventory 
        WHERE clinicId = ? AND category IS NOT NULL AND category != ''
