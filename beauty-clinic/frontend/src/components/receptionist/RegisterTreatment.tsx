// Path: frontend/src/components/receptionist/RegisterTreatment.tsx
...
export default function RegisterTreatment() {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const [patientsRes, doctorsRes, servicesRes, materialsRes, medicinesRes] = await Promise.all([
        axios.get('/api/v1/users', { params: { role: 'patient' } }),
        axios.get('/api/v1/doctors'),
        axios.get('/api/v1/services'),
        axios.get('/api/v1/treatments/materials'),
        axios.get('/api/v1/treatments/medicines')
      ])
      ...
      setDoctors(doctorsRes.data.map((d: any) => ({
        id: d.id,
        fullName: d.fullName,
        specialty: d.doctor?.specialty || 'متخصص',
        consultationFee: d.doctor?.consultationFee || 0
      })))
  ...
  const handleSubmitAndPrint = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        resetForm()
        const firstInput = document.querySelector('select') as HTMLSelectElement
        if (firstInput) firstInput.focus()
        toast.success('فرم برای ثبت درمان جدید آماده شد', {
          icon: '🔄',
          duration: 2000
        })
      }, 2500)
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-darkblue dark:text-white">ثبت درمان جدید</h1>
          {lastTreatmentId && (
        ...
        <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl flex items-center gap-3 animate-fadeIn">
          ...
          <div>
            ...
            <p className="text-sm text-green-600 dark:text-green-300">
              فاکتور در حال آماده‌سازی برای چاپ...
              {lastTreatmentId && ` (کد: ${lastTreatmentId})`}
            </p>
          </div>
        </div>
      )}
